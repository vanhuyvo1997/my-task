...
import { UserDetailsData } from "@/app/lib/dal/users-dal";
import { useFormStatus } from "react-dom";
import OneLineLimitedText from "../commons/limited-text";
import { changeUserStatusAction } from "@/app/lib/action/user-action";
...
export default function UserRow({ data, className }: Readonly<{ className?: string, data: UserDetailsData }>) {
    return <tr className={clsx(
        ...
        <td className="py-2 flex items-center gap-2 text-left">
            <span><Avatar selectable={false} diameter="37" avatarUrl={data.avatarUrl} /></span> {data.firstName + " " + data.lastName}
        </td>
        <td className="py-2 text-left"><OneLineLimitedText title={data.email}>{data.email}</OneLineLimitedText></td>
